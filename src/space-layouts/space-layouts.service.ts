...
export class SpaceLayoutsService {
  ...
  async requestTableDeletionPreview(
    venueId: string,
    ...
    if (!sourceLayout) {
      throw new BadRequestException(
        "Approved layout is required before requesting table deletion.",
      );
    }

    if (!this.findTableInLayout(sourceLayout, dto.tableId)) {
      throw new BadRequestException("Requested table was not found.");
    }
    ...
    const project = await this.prisma.spaceLayoutProject.create({
      data: {
        ownerId: sourceProject.ownerId,
        venueId: sourceProject.venueId,
        name: `${sourceProject.name ?? sourceProject.venue.name} - table deletion request`,
        status: SpaceLayoutStatus.PENDING_CHIN_CHIN_REVIEW,
    ...
    return {
      ...this.serializeProject(project),
      message: "Table deletion request submitted for Chin-Chin review.",
    };
  }
    ...
    try {
      ...
      await this.deviceTokensService.sendToUser({
        ...
        data: {
          ...
          tableId,
          tablePhotoId,
          approvalKind: "ADD_CHIN_CHIN_TABLE",
          approvalTitle,
          approvalMessage,
          tableLabel,
  ...
  private async notifyVenueTableUpdatesApproved(
    ...
    try {
      ...
      const previousLayout = this.asJsonObject(
        previousSavedLayout.layout as Prisma.JsonValue | null,
      );
      const approvedTable = this.findTableInLayout(
        approvedLayout,
        firstUpdate?.tableId ?? "",
      );
      const previousTable = this.findTableInLayout(
        previousLayout,
        firstUpdate?.tableId ?? "",
      );
      ...
      await this.deviceTokensService.sendToUser({
        ...
        data: {
          ...
          approvalKind: "UPDATE_CHIN_CHIN_TABLES",
          approvalTitle,
          approvalMessage,
          tableLabel,
          tableAction: firstUpdate?.action ?? "UPDATE",
          chinChinTier: approvedTier,
          tableType: approvedTier,
          seats: String(approvedSeats),
        },
      ...
      for (const table of tables) {
        ...
        if (chinChinTier === "VIP") {
          tableMap.seats = Math.max(
            this.numberFrom(tableMap.seats, requestedSeats ?? 12),
            requestedSeats ?? 12,
            12,
          );
          tableMap.maxPartySize = Math.max(
            this.numberFrom(tableMap.maxPartySize, requestedSeats ?? 12),
            requestedSeats ?? 12,
            12,
          );
          tableMap.minPartySize = Math.max(
            this.numberFrom(tableMap.minPartySize, 6),
            6,
          );
  ...
  private async mergeAdditionalRoomLayout(
    ...
    const normalizedIncomingRooms = incomingNewRooms.map((room, index) =>
      this.prepareIncomingAdditionalRoom(
        ...
        this.isTemporarySpaceForIncomingRoom(currentSpaceRooms, room, index),
      ),
    );
  ...
  private prepareIncomingAdditionalRoom(
    ...
    roomMap.isTemporarySpace =
      roomMap.isTemporarySpace === true || isTemporarySpace;
    const roomLabel =
  ...
  private isTemporarySpaceForIncomingRoom(
    spaceRooms: unknown[],
    incomingRoom: unknown,
    incomingRoomIndex: number,
  ) {
    ...
    return (
      ...
      !!matchingRoom &&
      !Array.isArray(matchingRoom) &&
      (matchingRoom as Record<string, unknown>).isTemporarySpace === true
    );
  }
  ...
  private normalizeSpace(space: SpaceShapeDto, index = 0) {
    return {
      ...
      features: {
        ...
        hasStage: Boolean(space.features?.hasStage),
  ...
  private applySpaceRoomMetadataToSuggestion(
    ...
    const nextSuggestion = JSON.parse(JSON.stringify(suggestion)) as Record<
      string,
      unknown
    >;
    const layoutOptions = Array.isArray(nextSuggestion.layoutOptions)
      ? nextSuggestion.layoutOptions
      : [];
    ...
    for (const option of layoutOptions) {
      ...
      const rooms = Array.isArray(optionMap.rooms) ? optionMap.rooms : [];
  ...
  private applyVipLabelOverridesToRoom(
    ...
    for (const table of tables) {
      ...
      tableMap.seats = Math.max(this.numberFrom(tableMap.seats, 12), 12);
      tableMap.maxPartySize = Math.max(
        this.numberFrom(tableMap.maxPartySize, 12),
        12,
      );
      tableMap.minPartySize = Math.max(
        this.numberFrom(tableMap.minPartySize, 6),
            ...
            },
            {
              ...
              text:
                normalizedVenueType === "CLUB"
                  ? "Venue type is CLUB. VIP tables are allowed and important. During OCR/text inspection, if you see uppercase VIP inside a round table/circle, that table must be returned as tableRole=CHIN_CHIN_TABLE, chinChinCandidate=true, chinChinTier=VIP, label=VIP, seats=12, minPartySize=6, and maxPartySize=12. VIP inside a circle has priority over generic ordinary-table rules."
        ...
        },
        {
          ...
          content: [
            ...
            },
            {
              ...
              text:
                normalizedVenueType === "CLUB"
                  ? "Club VIP OCR check: before final JSON, scan every round/circle table for the letters VIP drawn inside the circle. For each such table, force label=VIP, tableRole=CHIN_CHIN_TABLE, chinChinCandidate=true, chinChinTier=VIP, seats=12, minPartySize=6, maxPartySize=12, and tablePhotoStatus=MISSING_PHOTO if no photo id is available."
            ...
            {
              type: "input_text",
              text: "Setup feature flags are safety hints, not layout commands. If hasToilet, hasBar, hasStage, hasBilliardsOrDarts, hasTv, hasDjMusicCorner, hasStairs, or hasMainWalkway is true, look carefully for that label/object on the sketch and preserve it when visible. If the sketch does not show its position clearly, mention the uncertainty in notes instead of inventing a blocking fixture over tables or walkways.",
            },
            ...floorPlanContent,
          ],
        },
  ...
  private readSpaceDimensions(spaceJson: Prisma.JsonValue) {
    ...
    const lengthMeters =
      typeof sourceSpace.lengthMeters === "number"
        ...
        : 8;
    const isTemporarySpace = sourceSpace.isTemporarySpace === true;
